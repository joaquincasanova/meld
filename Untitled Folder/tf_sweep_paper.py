        ...
        for rnn in [True,False]:
            for subject_id in ['aud',7]:
                ...
                for treat in treats:
                    ...
                    name='/home/jcasa/meld/tnrse2017/subject_%s_pca_all_%s_rand_%s_cnn_%s_rnn_%s_locate_%s_treat_%s' % (subject_id, pca, rand_test, cnn, rnn,locate,lab_treat)
                    fname = name + '.csv' 
                    ...
                    with open(fname,'a') as csvfile:
                        ...
                        for [k_conv, n_conv1, n_conv2, n_lstm, n_layer, test_frac, val_frac, batch_frac] in params_list:
                            ...
                            with tf.Session() as session:
                                logdir = '/tmp/tensorflowlogs/tnrse2017/sub_%s/pca_all_%s/rand_%s/cnn_%s/rnn_%s/locate_knn_%s/treat_%s/' % (subject_id,pca,rand_test,cnn,rnn,locate,lab_treat)
