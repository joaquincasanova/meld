...
def cnn_2layers(img, kernel_shape_1, kernel_shape_2, bias_shape_1, bias_shape_2,dropout,beta):        
...
class meld:
    ...
    def cnn_nn(self):        
        with tf.variable_scope("cnn_nn"):
            ...
            with tf.name_scope('cnn'):
                cnn_out, reg1 = cnn_2layers(self.measfold, [self.k_conv, self.k_conv, self.n_chan_in, self.n_conv1],
                                            [self.k_conv, self.k_conv, self.n_conv1, self.n_conv2], [self.n_conv1],
                                            [self.n_conv2],self.dropoutPH, self.betaPH)
    ...
    def cnn_rnn(self):
        with tf.variable_scope("cnn_rnn"):
            ...
            # Store layers weight & bias
            with tf.name_scope('cnn'):
                cnn_out,reg1 = cnn_2layers(self.measfold, [self.k_conv, self.k_conv, self.n_chan_in, self.n_conv1], [self.k_conv, self.k_conv, self.n_conv1, self.n_conv2], [self.n_conv1], [self.n_conv2],self.dropoutPH, self.betaPH)
                dense_cnn = tf.reshape(cnn_out, [-1, self.n_dense]) # Reshape conv1 output to fit dense layer input

            with tf.name_scope('dense'):
                dense_out,reg2 = dense(dense_cnn, self.n_dense, self.n_out,self.dropoutPH, self.betaPH, 1.0)
