    ...
    def network(self):
        ...
        with tf.name_scope('cost'):
            ...
            if self.n_steps is None:
                ...
                reg=self.beta*tf.nn.l2_loss(wd)+self.beta*tf.nn.l2_loss(wc1)+self.beta*tf.nn.l2_loss(wc2)
                self.cross = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits, self.qtruePH_OH),name="cross")+reg
                self.accuracy = tf.reduce_mean(tf.cast(tf.equal(A,B),tf.float32),name="accuracy")+reg
                self.rmse = tf.sqrt(tf.reduce_mean(tf.square(tf.sub(self.qhat,self.qtruePH))),name="rmse")+reg

                self.cross_last = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits, self.qtruePH_OH),name="cross_last")+reg        
                self.accuracy_last = tf.reduce_mean(tf.cast(tf.equal(A,B),tf.float32),name="accuracy_last")+reg
                self.rmse_last = tf.sqrt(tf.reduce_mean(tf.square(tf.sub(self.qhat,self.qtruePH))),name="rmse_last")+reg
            else:
                ...
                self.cross = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits, qtrue_OH),name="cross")+reg
                self.accuracy = tf.reduce_mean(tf.cast(tf.equal(A,B),tf.float32),name="accuracy")+reg
                self.rmse = tf.sqrt(tf.reduce_mean(tf.square(tf.sub(self.qhat,qtrue_unflat))),name="rmse")+reg
                
                self.cross_last = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits_last, qtrue_last_OH),name="cross_last")+reg
                self.accuracy_last = tf.reduce_mean(tf.cast(tf.equal(AA,BB),tf.float32),name="accuracy_last")+reg
                self.rmse_last = tf.sqrt(tf.reduce_mean(tf.square(tf.sub(qtrue_last,qhat_last))),name="rmse_last")+reg
