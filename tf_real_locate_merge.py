...
for subject_id in [7,8]:
    meas_dims, m, p, n_steps, total_batch_size0 = nn_prepro.faces_dataset(subject_id,locate=True)
    total_batch_size+=total_batch_size0
...
subsample = 1
fname = './nn_real_locate_faces_merge_tune_%s.csv' % (subject_id)
                                        ...
                                        for n_conv1 in [2,3]:
                                            for n_conv2 in [5]:
                                                for n_layer in [2,3]:
                                                    ...
                                                    for n_lstm in [9,27]:
                                                        val_step=100
                                                        meas_img_test, qtrue_test, meas_dims, m, p, n_steps, test_size = nn_prepro.faces_dataset(test_id,selection=range(0,test_size_train),pca=True,subsample=subsample,justdims=False,cnn=True,locate=True)
                                                        #pick a test batch
                                                        print "Test batch"
                                                        print 0, test_size

                                                            choose = np.random.choice(total_batch_size_test,test_size,replace=False)
                                                            meas_img_test, qtrue_test, meas_dims, m, p, n_steps, test_size = nn_prepro.faces_dataset(test_id,selection=choose,pca=True,subsample=subsample,justdims=False,cnn=True,locate=True)
                                                            #pick a test batch
                                                            print "Test batch"
                                                            print choose

                                                        #pick a val batch
                                                        meas_img_val, qtrue_val, meas_dims, m, p, n_steps, val_size = nn_prepro.faces_dataset(train_id,selection=range((test_size_train),(test_size_train+val_size)),pca=True,subsample=subsample,justdims=False,cnn=True,locate=True)
                                                        print "Val batch"
                                                        print test_size_train, test_size_train+val_size

                                                        #pick a first batch of batch_size
                                                        batch_num=0
                                                        choose = np.random.choice(total_batch_size-test_size_train-val_size,batch_size,replace=False)
                                                        meas_img, qtrue, meas_dims, m, p, n_steps, batch_size = nn_prepro.faces_dataset(train_id,selection=(test_size_train+val_size+choose),pca=True,subsample=subsample,justdims=False,cnn=True,locate=True)
                                                        batch_num = 0
