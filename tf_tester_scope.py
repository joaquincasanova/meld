...
import meld_net_scope as meld_net
import csv
import nn_prepro
import time
...
for cnn in [True]:
    for rnn in [True]:
        for subject_id in ['aud']:
            ...
            fname = './data/Xsubject_%s_11x11_pca_%s_rand_%s_cnn_%s_rnn_%s.csv' % (subject_id, pca, rand_test, cnn, rnn)
            ...
            with open(fname,'a') as csvfile:
                ...
                for [n_conv1, n_conv2, n_lstm, n_layer, test_frac, val_frac, batch_frac] in params_list:
                    ...
                    with tf.Session() as session:
                        logdir = '/tmp/tensorflowlogs/scope/sub_%s/11x11/pca_%s/rand_%s/cnn_%s/rnn_%s/k_conv_%s' % (subject_id,pca,rand_test,cnn,rnn,k_conv)
                        ...
                        for batch_num in range(0,batches):
                            ...
                            while step<per_batch:
                                ...
                                if step % val_step==0 and step!=0:
                                    valid_summary,guess,true,costv = session.run([nn.valid_summary,nn.qhat,nn.qtrain_unflat, nn.cost], feed_dict={nn.qtrainPH: qtrue_val, nn.measPH: meas_img_val, nn.dropoutPH: dropout, nn.betaPH: beta})
