...
def conv_relu(img, kernel_shape, bias_shape, pool_shape,dropout):
    ...
    conv = tf.nn.conv2d(img, weights, strides=[1, 1, 1, 1], padding='SAME')
    relu = tf.nn.relu(conv + biases)

    return tf.nn.max_pool(relu, ksize=pool_shape, strides=pool_shape,padding='VALID')

def nn_logits(inputs, n_in,n_out,dropout):
    weights = tf.get_variable("weights", [n_in,n_out])
    biases = tf.get_variable("biases", [n_out])
    dense_out = tf.add(tf.matmul(inputs, weights),biases)
    return tf.nn.dropout(dense_out, dropout)
    ...
    def cnn_nn(self):        
        ...
        with tf.variable_scope('model', initializer = tf.random_normal_initializer()):
            with tf.variable_scope('CNN1'):
                ...
                cr1 = conv_relu(measfold, [self.k_conv, self.k_conv, self.n_chan_in, self.n_conv1], [self.n_conv1], [1, self.k_pool, self.k_pool, 1], self.dropoutPH)
            with tf.variable_scope('CNN2'):
                cr2 = conv_relu(cr1, [self.k_conv, self.k_conv, self.n_conv1, self.n_conv2], [self.n_conv2], [1, self.k_pool, self.k_pool, 1], self.dropoutPH)
    ...
    def rnn_nn(self):
        with tf.variable_scope('model', initializer = tf.random_normal_initializer()):
            ...
            with tf.variable_scope('rnn_layer'):
                last, outs = lstm(dense_out, self.n_lstm, self.n_layer, self.n_steps, self.dropoutPH)
            with tf.variable_scope('output_layer'):
                self.logits = nn_logits(outs, self.n_lstm,self.n_out,self.dropoutPH)
            with tf.variable_scope('output_last_layer'):
    ...
    def cost(self): 
        ...
        with tf.name_scope('cost'):
            if self.rnn is False:                
                ...
                self.cross = tf.add(tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(self.logits, qtrain_OH),name="cross"),0.0)
                self.accuracy = tf.reduce_mean(tf.cast(tf.equal(self.A,B),tf.float32),name="accuracy")
                self.rmse = tf.add(tf.sqrt(tf.reduce_mean(tf.square(tf.sub(self.qhat,self.qtrain_unflat))),name="rmse"),0.0)

                self.cross_last = tf.add(tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(self.logits, qtrain_OH),name="cross_last"),0.0)        
                self.accuracy_last = tf.reduce_mean(tf.cast(tf.equal(self.A,B),tf.float32),name="accuracy_last")
                self.rmse_last = tf.add(tf.sqrt(tf.reduce_mean(tf.square(tf.sub(self.qhat,self.qtrainPH))),name="rmse_last"),0.0)
                if self.n_out==3:
                    self.cost = self.rmse 
                else:
                ...
                self.cross_last = tf.add(tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(self.logits_last, qtrain_last_OH),name="cross_last"),0.0)
                self.accuracy_last = tf.reduce_mean(tf.cast(tf.equal(self.AA,BB),tf.float32),name="accuracy_last")
                self.rmse_last = tf.add(tf.sqrt(tf.reduce_mean(tf.square(tf.sub(qtrain_last,self.qhat_last))),name="rmse_last"),0.0)
                if self.n_out==3:
