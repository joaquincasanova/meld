...
for train_id in [8,7]:
    for test_id in [7,8]:
        ...
        for e in [1,4]:
            ...
            test_rmse_last=np.mean(data[test,4])
            val = np.where(data[:,col_step]==-1)
            val_rmse_last=np.mean(data[val,4])
            lab='Trained on: '+str(train_id)+' Tested on: '+str(test_id)+'val rmse='+str(val_rmse_last)+' test rmse='+str(test_rmse_last)
